...
RSpec.describe Admin::GalleryImagesController, :type => :controller do
  render_views
  ...
  describe '#edit' do
    context "when gallery image exists" do
      it "should return gallery image to edit" do
        ...
        expect(response).to render_template("admin/gallery_images/edit")
        expect(assigns(:gallery_image)).to eq(second_image)
      end
    end
  ...
  describe '#create' do
    context "when gallery exists" do  
      it "should create new gallery image" do
        ...
        expect(response).to redirect_to admin_gallery_url(gallery)
